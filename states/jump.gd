...
func enter() -> void:
	super()
	parent.velocity.y = jump_velocity # Add jump velocity
...
func process_physics(delta: float) -> State:
	...
	if parent.holding_jump and parent.move_component.try_hold_jump():
		...
		if parent.jump_time < max_jump_hold_time:
			parent.velocity.y += hold_jump_accel * delta
		else:
